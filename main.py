...
# ── Solution Trail Logger ──────────────────────────────────────────
class TrailLogger:
    """Consistent solution trail logger with auto-numbered steps"""
    ...
    def _init_trail(self):
        """Initialize trail with header"""
        self.trail.append(("h", f"{self.method_name.upper()} - SOLUTION TRAIL"))
        self.trail.append(("", ""))
    
    def section(self, heading):
        """Add a section heading"""
        self.trail.append(("s", heading))
    
    def add_step(self, content, tag="b"):
        """Add a numbered step with auto-increment"""
        self.step_count += 1
        self.trail.append((tag, f"  Step {self.step_count}: {content}"))
    
    def add_line(self, content, tag="b"):
        """Add a line with optional tag (no numbering)"""
        self.trail.append((tag, content))
    ...
    def get_trail(self):
        """Get the complete trail as list of tuples"""
        return self.trail
...
def make_trail_trap(raw, expr, a, b, n, result):
    ...
    log = TrailLogger("Trapezoidal Rule")
    
    # GIVEN section
    log.section("GIVEN")
    log.add_line(f"  f(x) = {raw}")
    log.add_line(f"  a = {a},  b = {b},  n = {n}")
    log.blank()
    
    # METHOD section
    log.section("METHOD")
    log.add_line("  Integral ~ (h/2) x [f(x0) + 2f(x1) + ... + 2f(xn-1) + f(xn)]")
    log.add_line("  Endpoint coefficients = 1,  Interior = 2")
    log.blank()
    
    # STEPS section
    log.section("STEPS")
    log.add_step(f"Calculate interval width h = (b - a) / n  =  ({b} - {a}) / {n}")
    log.add_line(f"  h = {h:.8f}")
    log.blank()
    
    log.add_step("Evaluate function at all points")
    log.add_line(f"  f(x0) = f({a}) = {ys[0]:.8f}   [coeff=1]")
    for i in range(1, min(n, 8)+1):
        log.add_line(f"  f(x{i}) = f({xs[i]:.5f}) = {ys[i]:.8f}   [coeff=2]")
    if n > 8:
        log.add_line(f"  ... {n-8} more interior points ...", tag="d")
    log.add_line(f"  f(x{n}) = f({b}) = {ys[-1]:.8f}   [coeff=1]")
    log.blank()
    
    log.add_step(f"Calculate weighted sum")
    log.add_line(f"  Sum = {ys[0]:.8f} + 2 x {int_sum:.8f} + {ys[-1]:.8f}")
    log.add_line(f"  Sum = {weighted:.8f}", tag="r")
    log.blank()
    
    log.add_step(f"Multiply by h/2 to get final result")
    log.add_line(f"  Result = (h/2) x Sum  =  ({h:.6f}/2) x {weighted:.6f}")
    log.add_line(f"  Result = {result:.10f}", tag="r")
    log.blank()
    
    # FINAL section
    log.section("FINAL")
    log.add_line(f"  Integral from {a} to {b} of ({raw}) dx", tag="r")
    log.add_line(f"  = {result:.10f}", tag="r")
    log.blank()
    
    # VERIFICATION section
    log.section("VERIFICATION")
    _append_verify_to_logger(log, trap, expr, raw, a, b, n, result)
    
    return log.get_trail()
...
def make_trail_s13(raw, expr, a, b, n, result):
    ...
    log = TrailLogger("Simpson's 1/3 Rule")
    
    # GIVEN section
    log.section("GIVEN")
    log.add_line(f"  f(x) = {raw}")
    log.add_line(f"  a = {a},  b = {b},  n = {n}  (must be even)")
    log.blank()
    
    # METHOD section
    log.section("METHOD")
    log.add_line("  Integral ~ (h/3) x [f(x0) + 4f(x1) + 2f(x2) + 4f(x3) + ... + f(xn)]")
    log.add_line("  Pattern: 1, 4, 2, 4, 2, ..., 4, 1")
    log.blank()
    
    # STEPS section
    log.section("STEPS")
    log.add_step(f"Verify n is even")
    log.add_line(f"  n = {n}  -> {'OK (even)' if n%2==0 else 'adjusted'}")
    log.blank()
    
    log.add_step(f"Calculate interval width h = (b - a) / n")
    log.add_line(f"  h = ({b} - {a}) / {n} = {h:.8f}")
    log.blank()
    
    log.add_step("Evaluate function with coefficients")
    for i in range(min(n+1, 9)):
        log.add_line(f"  f(x{i}) = f({xs[i]:.5f}) = {ys[i]:.8f}   [coeff={coeffs[i]}]")
    if n+1 > 9:
        log.add_line(f"  ... {n+1-9} more points ...", tag="d")
    log.blank()
    
    log.add_step(f"Calculate weighted sum")
    log.add_line(f"  Weighted sum = {weighted:.8f}", tag="r")
    log.blank()
    
    log.add_step(f"Multiply by h/3 to get final result")
    log.add_line(f"  Result = (h/3) x {weighted:.6f}  =  ({h:.6f}/3) x {weighted:.6f}")
    log.add_line(f"  Result = {result:.10f}", tag="r")
    log.blank()
    ...
    # VERIFICATION section
    log.section("VERIFICATION")
    _append_verify_to_logger(log, s13, expr, raw, a, b, n, result)
...
def make_trail_mid(raw, expr, a, b, n, result):
    ...
    # METHOD section
    log.section("METHOD")
    log.add_line("  Integral ~ h x SUM of f(midpoints)")
    log.add_line("  Midpoint of interval i = a + (i + 0.5) x h")
    log.blank()
    ...
    log.add_step("Evaluate function at midpoints")
    for i in range(min(n, 8)):
        log.add_line(f"  Interval {i}: mid={mids[i]:.6f}  f={ys[i]:.8f}")
    if n > 8:
        log.add_line(f"  ... {n-8} more intervals ...", tag="d")
    log.blank()
    
    log.add_step(f"Calculate sum of all function values")
    log.add_line(f"  SUM = {sum(ys):.8f}", tag="r")
    log.blank()
    
    log.add_step(f"Multiply by h to get final result")
    log.add_line(f"  Result = {h:.8f} x {sum(ys):.8f}")
    log.add_line(f"  Result = {result:.10f}", tag="r")
    log.blank()
    ...
    # VERIFICATION section
    log.section("VERIFICATION")
    _append_verify_to_logger(log, mid, expr, raw, a, b, n, result)
...
def make_trail_s38(raw, expr, a, b, n, result):
    ...
    log = TrailLogger("Simpson's 3/8 Rule")
    
    # GIVEN section
    log.section("GIVEN")
    log.add_line(f"  f(x) = {raw}")
    log.add_line(f"  a = {a},  b = {b},  n = {n}  (multiple of 3)")
    log.blank()
    
    # METHOD section
    log.section("METHOD")
    log.add_line("  Integral ~ (3h/8) x [f(x0)+3f(x1)+3f(x2)+2f(x3)+...+f(xn)]")
    log.add_line("  Pattern: 1, 3, 3, 2, 3, 3, 2, ..., 1")
    log.blank()
    ...
    log.add_step(f"Calculate weighted sum and multiply by 3h/8")
    log.add_line(f"  Weighted sum = {weighted:.8f}")
    log.add_line(f"  Result = (3 x {h:.6f} / 8) x {weighted:.6f}")
    log.add_line(f"  Result = {result:.10f}", tag="r")
    log.blank()
    ...
    # VERIFICATION section
    log.section("VERIFICATION")
    _append_verify_to_logger(log, s38, expr, raw, a, b, n, result)
...
def _append_verify_to_logger(log, fn, expr, raw, a, b, n, result):
    """Add verification section to TrailLogger"""
    f = make_f(expr)
    r2 = fn(f, a, b, n*2)
    ref, _ = sci_integrate.quad(f, a, b)
    abs_err = abs(result - ref)
    ...
    log.separator()
    log.add_line("VERIFICATION & ACCURACY AUDIT", tag="v")
    log.blank()
    log.add_line("  [1] Richardson Extrapolation:")
    log.add_line(f"      n={n}: {result:.10f}")
    log.add_line(f"      n={n*2}: {r2:.10f}")
    log.add_line(f"      Estimated error = {rich_err:.4e}")
    log.blank()
    log.add_line("  [2] SciPy adaptive quadrature:")
    log.add_line(f"      Reference = {ref:.10f}")
    log.add_line(f"      Absolute error = {abs_err:.4e}")
    rel = abs_err/abs(ref)*100 if ref != 0 else 0
    log.add_line(f"      Relative error = {rel:.6f}%")
    log.blank()
    ...
    try:
        ...
        ai = sp.integrate(ex, x)
        exact = float(ai.subs(x, b) - ai.subs(x, a))
        log.add_line("  [3] SymPy exact symbolic:")
        log.add_line(f"      Antiderivative = {ai}")
        log.add_line(f"      Exact value    = {exact:.10f}")
        log.add_line(f"      Error vs exact = {abs(result-exact):.4e}")
        log.blank()
    except Exception:
        pass
